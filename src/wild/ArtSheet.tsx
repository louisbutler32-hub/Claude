...
import { WILD_NAME, WILD_PHOTO_ART, WILD_PHOTO_CREDITS } from "./wild";

export const WildArtSheet: React.FC = () => {
  loadVeggieFonts();
  usePhotoArt("wild", WILD_PHOTO_CREDITS);
  const ids = Object.keys(WILD_PHOTO_ART);
  return (
    <AbsoluteFill style={{ backgroundColor: "#eef4ea" }}>
      <SceneFilters />
      <svg width={1920} height={1080} style={{ position: "absolute", inset: 0 }}>
        {ids.map((id, i) => {
          const Art = WILD_PHOTO_ART[id];
          const col = i % 6;
