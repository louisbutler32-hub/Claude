...
/**
 * Episode thumbnails, built from the same art the videos use so a frame
 * grab and the thumbnail can never drift apart.
 *
 *   A — the format's hook: three shadows and a question
 *   B — the payoff: the finished board
 */

export type ThumbConfig = {
  subject: GuessSubject;
  /** the word after "GUESS THE" */
  noun: string;
  /** three ids to cast as shadows, with their x and scale */
  heroes: { id: string; x: number; scale: number }[];
  /** headline for the board variant */
  boardLine: string;
};
...
export const BoardThumb: React.FC<{ config: ThumbConfig }> = ({ config }) => {
  loadVeggieFonts();
  return (
    <AbsoluteFill style={{ backgroundColor: "#dceaee", overflow: "hidden" }}>
      <SceneFilters />
      ...
      >
        <Board
          ...
          solved={config.subject.boardOrder}
          riseAt={-100}
          popAt={-100}
          landAt={-100}
          exitAt={100000}
      ...
      <PunchLine text={config.boardLine} colors={TITLE_COLORS} size={136} top={802} />
      <PaperGrain opacity={0.12} />
    </AbsoluteFill>
  );
};
