...
what if texas stayed a country, republic of texas, texas independence, texas history, 1836, 1845, texas annexation, alternate history, texas economy, lone star, mexican american war, american history, maps, history shorts, geo shorts, map facts, learning, what if history, texas explained, texas facts

## Thumbnail

`out/thumbnail-geo-texas.jpg`, 1080×1920 — built with `npm run geo:texas:thumb`.
...
| Music | "Accralate" by Kevin MacLeod, CC BY 4.0 — the credit block above must stay in the description. The bed is a local drop-in at `public/audio/geo-texas-music.mp3` (cut with `python3 scripts/make-geo-music.py texas <track.mp3>`), not in the repo. Render with `npm run geo:texas:music`. |
